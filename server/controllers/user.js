...
exports.loginUser = async function(req,res,next) {
	try{
		console.log('여기')
		let userIdCheckdata = { userId: req.body.userId };
		console.log(userIdCheckdata);
		let userInfo = await UserController.getUserFindIdLogin(userIdCheckdata);
		let favoriteCheck;
		if(userInfo == null) {
			output.msg = "fail";
...
exports.pushOnOff = async function(req,res,next) {
	try{
		...
		let userIdCheckdata = { _id: userToken.ObjectId };
		let userInfo = await UserController.getUserFindObjectId(userIdCheckdata);
		let updateUserPushOnOffData = {"$set":{"userPushOnOff": req.body.userPushOnOff}};
