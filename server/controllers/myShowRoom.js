...
exports.main = async function(req, res, next) {
	try {
		...
		if(req.headers.usertoken == "guest"){
		    ...
		    // return console.log(userCategoryTagInfo.userFavorite)
		    if(userCategoryTagInfo.userFavorite == 0){
		    	...
		    	console.log(userCategoryTagInfo.userLocal)
			    userDegree = await dbModel.dbOverlap2('select * from weather where city = ?',userCategoryTagInfo.userLocal); //온도 
		...
		let data = [
			{
				"$project": {
					_id: "$_id._id",
					codyPhoto1: {"$split": ["$_id.photo", "origin"]}
				}
			},
			{
				"$unwind": "$codyPhoto1"
			...
			},
			{
				"$project": {
					_id: 1,
					photo: {"$concat": ["$codyPhotofirst","thumbnail","$codyPhotolast"]}
				}
			},
			{"$limit": 10},{"$sort": {"value" : -1}}
		]
		db.codyRanking.aggregate(data,function(err,result){
			if(result == 0){
				result =null;
				resolve(result);
			}else{
...
// 10대 순위 
 function findCody10Rank(){
 	return new Promise(function(resolve, reject){
		data = [	
			{
				"$project": {
					_id: "$_id._id",
					codyPhoto: {"$split": ["$_id.photo", "origin"]}
				}
			},
			{
				"$unwind" :  "$codyPhoto" 
			},
			{
				"$group" : {
					_id : "$_id",
					codyPhotoFirst : { "$first" : "$codyPhoto" },
					codyPhotoLast : { "$last" : "$codyPhoto" }
				}
			},
			{
				"$project" : {
		...
		db.cody10Ranking.aggregate(data,function(err,result){
			if(result == 0){
				codyData =null;
				resolve(result);
			}else{
...
// 20대 순위 
 function findCody20Rank(){
 	return new Promise(function(resolve, reject){
		...
		db.cody20Ranking.aggregate(data,function(err,result){
			if(result == 0){
				codyData =null;
				resolve(result);
			}else{
...
// 30대 순위 
 function findCody30Rank(){
 	return new Promise(function(resolve, reject){
		...
		db.cody30Ranking.aggregate(data,function(err,result){
			if(result == 0){
				codyData =null;
				resolve(result);
			}else{
 	...
 	return new Promise(function(resolve, reject){
		data = [	
			...
			},
			{
				"$project" : {
					_id: 1,
					photo : { "concat" : ["$codyPhotoFirst","thumbnail","$codyPhotoFirst"]}
				}
			},
			{"$limit": 10},{"$sort": {"value" : -1}}
		...
		db.cody40Ranking.aggregate(data,function(err,result){
			if(result == 0){
				codyData =null;
				resolve(result);
			}else{
