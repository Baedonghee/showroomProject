	...
	try{
        if(req.headers.usertoken == null){
			...
			if(!clothCode & !textSearch){
				sql = sql + viewQuery + ' limit 20 offset ?'
			}else if(!clothCode || textSearch){
				sql = sql + textQuery + viewQuery + ' limit 20 offset ?'
			}else if(!textSearch || clothCode){
				sql = sql +' where ( '+ sQuery + mQuery + bQuery +' ) ' + viewQuery + ' limit 20 offset ?'
			}else{
				sql = sql+' where ( '+ sQuery+mQuery + bQuery +' ) '+' and ( '+ textQuery + ' ) '+viewQuery + ' limit 20 offset ?'
			}
